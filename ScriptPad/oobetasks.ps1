...
$Transcript = "$((Get-Date).ToString('yyyy-MM-dd-HHmmss'))-SystemTasks.log"
...
function Step-oobeSetRegionLanguage {
    [CmdletBinding()]
    param ()
    if ($Global:oobeCloud.oobeSetRegionLanguage -eq $true) {
...
function Step-oobeSetDateTime {
    [CmdletBinding()]
    param ()
    if ($Global:oobeCloud.oobeSetDateTime -eq $true) {
...
function Step-oobeStopComputer {
    [CmdletBinding()]
    param ()
    if ($Global:oobeCloud.oobeStopComputer -eq $true) {
        Write-Host -ForegroundColor Cyan 'Build Complete!'
